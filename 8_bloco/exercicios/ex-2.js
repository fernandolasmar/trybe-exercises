const numberChecher = (myNumber, number) => myNumber === number;

const lotteryRes = (myNumber, callback) => {
  const number = Math.floor((Math.random() * 5) +1);

  return callback(myNumber, number) ? 'Parabéns, você ganhou!' : 'Que pena, tente novamente!';
};

console.log(lotteryRes(3, numberChecher));
